...
function initialize() {
  ...
  var map = new google.maps.Map(document.getElementById('map-canvas'), mapOptions);
      ...
      <%if(session.getAttribute("searchObj")!=null)
      {
    	  AgentAddSearchCriteria crit=((AgentAddSearchCriteria)session.getAttribute("searchObj"));
    	  if(crit.getSearchedList()!=null && crit.getSearchedList().size()>0)
    	  {
    	  List<AdvertsimentInfo> aList=(List<AdvertsimentInfo>)crit.getSearchedList();
    	  int counter=1;
...
<body>
	...
	<div id="map-canvas" >slmndsdln</div>
				...
				<%
					AgentAddSearchCriteria searchInfo = null;
					if (session.getAttribute("searchObj") != null) {
						searchInfo = (AgentAddSearchCriteria) session
								.getAttribute("searchObj");
					}
				%>
				...
				<div class="row-fluid">
					<ul class="thumbnails">
						...
						<li class="span4">
							...
							<div class="thumbnail">
								<img src="images/<%=addList.get(i).getImgLocation().get(0)%>"
									width="100" height="100">
								<h3><%=addList.get(i).getPropertyType()%></h3>
								<h4><%=addList.get(i).getPropertyInfo()%></h4>
								<h4>Price: $<%=addList.get(i).getPrice(	)%></h4>
								<form class="" method="get" action="VAdd">
									<input type="hidden"
